...
const ML_PK_SIZE: usize = 2592; // Approximate size for ML-DSA-87 public key
const ED_PK_SIZE: usize = 32;

const BINDING_TAG_SIZE: usize = 32;

const ML_SIG_SIZE: usize = 4627; // Exact size for ML-DSA-87 signature
pub const VERIFYING_KEY_SIZE: usize = ML_PK_SIZE + ED_PK_SIZE;

#[derive(Clone)]
pub struct XwingSig87VerifyingKey {
    vk_ml: MLDSA87VerificationKey,
    vk_ed: EdVerifyingKey,
}

impl PartialEq for XwingSig87VerifyingKey {
...
pub struct XwingSig87SigningKey {
    seed: [u8; MASTER_SEED_SIZE],
}

impl XwingSig87SigningKey {
...
#[derive(Clone)]
pub struct XwingSig87Signature {
    sig_ml: MLDSA87Signature,
    sig_ed: EdSignature,
    binding_tag: [u8; BINDING_TAG_SIZE],
}

impl PartialEq for XwingSig87Signature {
    fn eq(&self, other: &Self) -> bool {
        self.to_bytes() == other.to_bytes()
    }
}

impl XwingSig87VerifyingKey {
    ...
    pub fn verify(&self, message: &[u8], signature: &XwingSig87Signature) -> Result<(), Error> {
...
impl From<&[u8; VERIFYING_KEY_SIZE]> for XwingSig87VerifyingKey {
...
impl XwingSig87SigningKey {
    ...
    ) -> Result<XwingSig87Signature, Error> {
    ...
    pub fn verifying_key(&self) -> XwingSig87VerifyingKey {
        let (kp_ml, sk_ed) = expand_seed(&self.seed);
        let vk_ml = kp_ml.verification_key;
        let vk_ed = sk_ed.verifying_key();
        XwingSig87VerifyingKey { vk_ml, vk_ed }
    }
}

impl XwingSig87Signature {
    pub fn to_bytes(&self) -> Vec<u8> {
...
impl TryFrom<&[u8]> for XwingSig87Signature {
    type Error = Error;
...
pub fn generate_keypair<R: CryptoRng + RngCore>(
    ...
    let vk_ed = sk_ed.verifying_key();
    (
        XwingSig87SigningKey::new(seed),
        XwingSig87VerifyingKey { vk_ml, vk_ed },
    )
}

fn expand_seed(seed: &[u8; MASTER_SEED_SIZE]) -> (MLDSA87KeyPair, EdSigningKey) {
