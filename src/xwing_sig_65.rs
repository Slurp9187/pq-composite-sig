...
#[derive(Clone)]
pub struct XwingSig65VerifyingKey {
    vk_ml: MLDSA65VerificationKey,
    vk_ed: EdVerifyingKey,
}

impl PartialEq for XwingSig65VerifyingKey {
...
pub struct XwingSig65SigningKey {
    seed: [u8; MASTER_SEED_SIZE],
}

impl XwingSig65SigningKey {
...
#[derive(Clone)]
pub struct XwingSig65Signature {
    sig_ml: MLDSA65Signature,
    sig_ed: EdSignature,
    binding_tag: [u8; BINDING_TAG_SIZE],
}

impl PartialEq for XwingSig65Signature {
    fn eq(&self, other: &Self) -> bool {
        self.to_bytes() == other.to_bytes()
    }
}

impl XwingSig65VerifyingKey {
    ...
    pub fn verify(&self, message: &[u8], signature: &XwingSig65Signature) -> Result<(), Error> {
...
impl From<&[u8; VERIFYING_KEY_SIZE]> for XwingSig65VerifyingKey {
...
impl XwingSig65SigningKey {
    ...
    ) -> Result<XwingSig65Signature, Error> {
    ...
    pub fn verifying_key(&self) -> XwingSig65VerifyingKey {
        let (kp_ml, sk_ed) = expand_seed(&self.seed);
        let vk_ml = kp_ml.verification_key;
        let vk_ed = sk_ed.verifying_key();
        XwingSig65VerifyingKey { vk_ml, vk_ed }
    }
}

impl XwingSig65Signature {
    pub fn to_bytes(&self) -> Vec<u8> {
...
impl TryFrom<&[u8]> for XwingSig65Signature {
    type Error = Error;
...
pub fn generate_keypair_xwing_sig_65<R: CryptoRng + RngCore>(
    rng: &mut R,
) -> (XwingSig65SigningKey, XwingSig65VerifyingKey) {
